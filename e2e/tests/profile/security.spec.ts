...
test.describe('Profil — Onglet Sécurité', () => {
  ...
  // ----------------------------------------------------------
  test('onglet Sécurité accessible et section visible', async ({ memberPage }) => {
  ...
  // ----------------------------------------------------------
  test("section changement d'email → formulaire visible", async ({ memberPage }) => {
    await memberPage.goto('/profile');
    await memberPage.locator('#tab-security').click();
    await memberPage.locator('[data-testid="profile-security-tab"]').waitFor({ state: 'visible', timeout: 10_000 });
  ...
  // ----------------------------------------------------------
  test('email invalide → bouton soumission désactivé', async ({ memberPage }) => {
    await memberPage.goto('/profile');
    await memberPage.locator('#tab-security').click();
    await memberPage.locator('[data-testid="profile-security-tab"]').waitFor({ state: 'visible', timeout: 10_000 });

    await memberPage.locator('[data-testid="input-new-email"]').fill('not-a-valid-email');
    await memberPage.locator('[data-testid="input-confirm-email"]').fill('not-a-valid-email');
  ...
  // ----------------------------------------------------------
  test('email valide → confirmation envoyée (toast succès)', async ({ memberPage }) => {
    await memberPage.goto('/profile');
    await memberPage.locator('#tab-security').click();
    await memberPage.locator('[data-testid="profile-security-tab"]').waitFor({ state: 'visible', timeout: 10_000 });

    const uniqueEmail = `new-email-${Date.now()}@example.com`;
    await memberPage.locator('[data-testid="input-new-email"]').fill(uniqueEmail);
    await memberPage.locator('[data-testid="input-confirm-email"]').fill(uniqueEmail);
  ...
  // ----------------------------------------------------------
  test('sessions actives → liste avec au moins 1 session', async ({ memberPage }) => {
    await memberPage.goto('/profile');
    await memberPage.locator('#tab-security').click();
    await memberPage.locator('[data-testid="profile-security-tab"]').waitFor({ state: 'visible', timeout: 10_000 });
  ...
  // ----------------------------------------------------------
  test('révoquer une session → session disparaît', async ({ memberPage, db }) => {
    ...
    if (count > 1) {
      ...
      if (id) {
        const responsePromise = memberPage.waitForResponse(
          (resp) => resp.url().includes('/api/auth/sessions/') && resp.request().method() === 'DELETE',
          { timeout: 10_000 },
        );
