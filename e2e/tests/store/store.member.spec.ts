...
test.describe("Boutique — Flux membre", () => {
  ...
  test("charger /store → boutique membre visible", async ({ memberPage }) => {
    ...
    await expect(
      memberPage.locator('[data-testid="store-page"]'),
    ).toBeVisible();
    await expect(
      memberPage.locator('[data-testid="tab-boutique"]'),
    ).toBeVisible();
    await expect(
      memberPage.locator('[data-testid="tab-mes-commandes"]'),
    ).toBeVisible();
  });
  ...
  // ----------------------------------------------------------
  test("onglet Boutique → produits listés ou état vide", async ({
    memberPage,
  }) => {
    await memberPage.goto("/store");
    await memberPage
  ...
  // ----------------------------------------------------------
  test("ajouter un article au panier → bouton voir panier visible", async ({
    memberPage,
    db,
  }) => {
    // Nettoyer les anciens articles de test pour éviter la pagination
    await db
      .query("DELETE FROM articles WHERE nom LIKE 'Article Panier%'")
      .catch(() => {});
    ...
    try {
      ...
      // Attendre que l'article soit visible (waitFor réessaye, contrairement à isVisible)
      const articleCard = memberPage.locator(
        `[data-testid="article-boutique-${articleId}"]`,
      );
      const isVisible = await articleCard
      ...
      if (isVisible) {
        await memberPage
          .locator(`[data-testid="btn-order-${articleId}"]`)
          .click();
  ...
  // ----------------------------------------------------------
  test("onglet Mes Commandes → historique ou état vide visible", async ({
    memberPage,
  }) => {
    await memberPage.goto("/store");
    await memberPage
  ...
  test("onglets admin non visibles pour un membre", async ({ memberPage }) => {
    ...
    await expect(
      memberPage.locator('[data-testid="tab-catalogue"]'),
    ).not.toBeVisible();
    await expect(
      memberPage.locator('[data-testid="tab-commandes"]'),
    ).not.toBeVisible();
  });
});
