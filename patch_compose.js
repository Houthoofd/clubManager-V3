...
// We need to add useRef to imports.
...
// Replace envoyeParEmail with sendMethod
content = content.replace(
...
// 6. Replace the old "Envoi par email" checkbox with the "Mode d'envoi" section
const modeEnvoiHTML = `
          {/* Mode d'envoi */}
          <fieldset>
...
content = content.replace(
    /\{\/\*\s+"\?"\?\s+Envoi par email[\s\S]*?\{canBroadcast && \([\s\S]*?\)\}/,
    modeEnvoiHTML
);
