...
export function CreateAlertModal({ isOpen, onClose, alertTypes }: CreateAlertModalProps) {
    ...
    createAlert(
      ...
      },
      {
        onSuccess: () => {
          ...
          setForm({ user_id: '', alerte_type_id: '', notes: '', donnees_contexte: '' });
          onClose();
        },
        onError: () => {
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              {t('userAlerts.modal.fields.userId')} <span className="text-red-500">*</span>
            </label>
            <input
              type="number"
              value={form.user_id}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              {t('userAlerts.modal.fields.type')} <span className="text-red-500">*</span>
            </label>
            <select
              value={form.alerte_type_id}
              onChange={(e) => setForm((f) => ({ ...f, alerte_type_id: e.target.value }))}
              required
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              <option value="">-- Choisir un type --</option>
              {alertTypes.filter((at) => at.actif).map((at) => (
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) => setForm((f) => ({ ...f, donnees_contexte: e.target.value }))}
              rows={3}
              placeholder={'{}'}
