...
import { useAlertTypes, useAdminAlerts, useMyAlerts, useDeleteAlertType, useResolveAlert, useIgnoreAlert } from '../hooks/useAlerts';
import type { AlertStatut, AlertPriorite, AlertTypeDto, AlertUserDto } from '../api/alertsApi';
...
function AdminTab() {
  ...
  const { data: alertTypes = [], isLoading: typesLoading, isError: typesError } = useAlertTypes();
  ...
  const { data: adminAlerts = [], isLoading: alertsLoading, isError: alertsError } = useAdminAlerts(adminFilters);
  ...
  return (
    <div className="space-y-6">
      ...
      {section === 'types' && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h2 className="text-base font-semibold text-gray-900">{t('alertTypes.title')}</h2>
            <button
          ...
          {typesLoading ? (
            ...
            <div className="px-6 py-8 text-center text-sm text-red-500">{t('errors.loadFailed')}</div>
          ) : alertTypes.length === 0 ? (
            <div className="px-6 py-12 text-center">
              <p className="text-sm font-medium text-gray-900">{t('alertTypes.empty.title')}</p>
              <p className="text-sm text-gray-500 mt-1">{t('alertTypes.empty.description')}</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('alertTypes.columns.code')}</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('alertTypes.columns.name')}</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('alertTypes.columns.priority')}</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('alertTypes.columns.active')}</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('alertTypes.columns.actions')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {alertTypes.map((at) => (
                    <tr key={at.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 font-mono text-xs text-gray-700">{at.code}</td>
                      <td className="px-4 py-3">
                        <p className="font-medium text-gray-900">{at.nom}</p>
                        {at.description && <p className="text-xs text-gray-500 mt-0.5 truncate max-w-xs">{at.description}</p>}
                      </td>
                      <td className="px-4 py-3"><AlertTypeBadge priorite={at.priorite} size="sm" /></td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${at.actif ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                          {at.actif ? 'Oui' : 'Non'}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            onClick={() => { setEditingType(at); setIsTypeModalOpen(true); }}
      ...
      {section === 'alerts' && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h2 className="text-base font-semibold text-gray-900">{t('userAlerts.title')}</h2>
            <button
          ...
          <div className="flex items-center gap-3 px-6 py-3 border-b border-gray-200 bg-gray-50">
            ...
            <select
              ...
              onChange={(e) => setPriorityFilter(e.target.value as AlertPriorite | '')}
              className="px-3 py-1.5 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
            >
          ...
          {alertsLoading ? (
            ...
            <div className="px-6 py-12 text-center">
              <p className="text-sm font-medium text-gray-900">{t('userAlerts.empty.title')}</p>
              <p className="text-sm text-gray-500 mt-1">{t('userAlerts.empty.description')}</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('userAlerts.columns.user')}</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('userAlerts.columns.type')}</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('userAlerts.columns.priority')}</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('userAlerts.columns.status')}</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('userAlerts.columns.detected')}</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('userAlerts.columns.actions')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
...
function AlertTableRow({ alert, onViewActions, formatDate }: AlertTableRowProps) {
  const { t } = useTranslation('alerts');
  ...
  return (
    <tr className="hover:bg-gray-50 transition-colors">
      ...
      <td className="px-4 py-3">
        <p className="font-medium text-gray-900">{alert.alerte_type?.nom ?? `#${alert.alerte_type_id}`}</p>
        {alert.alerte_type && <p className="text-xs text-gray-400 font-mono">{alert.alerte_type.code}</p>}
      </td>
      <td className="px-4 py-3"><AlertTypeBadge priorite={priorite} size="sm" /></td>
      <td className="px-4 py-3"><AlertStatusBadge statut={alert.statut} size="sm" /></td>
      <td className="px-4 py-3 text-xs text-gray-500 whitespace-nowrap">{formatDate(alert.date_detection)}</td>
      <td className="px-4 py-3">
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-8 text-sm text-red-500">{t('errors.loadFailed')}</div>
      ) : myAlerts.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm px-6 py-12 text-center">
          ...
          <p className="text-sm font-medium text-gray-900">{t('myAlerts.empty.title')}</p>
          <p className="text-sm text-gray-500 mt-1">{t('myAlerts.empty.description')}</p>
        </div>
      ) : (
...
export function AlertsPage() {
  ...
  const [activeTab, setActiveTab] = useState<MainTab>(isAdmin ? 'admin' : 'myAlerts');
