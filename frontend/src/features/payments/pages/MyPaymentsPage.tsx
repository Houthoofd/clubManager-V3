...
function getStatusBadge(statut: string): { label: string; color: BadgeColor } {
  switch (statut) {
    ...
    case "rembourse":
      return { label: "Remboursé", color: "blue" };
    case "en_retard":
...
export const MyPaymentsPage: React.FC = () => {
  ...
  // ── State ──────────────────────────────────────────────────────────────────
  const [activeTab, setActiveTab] = useState<"payments" | "schedules">(
    "payments",
  );
          ...
          <nav className="flex gap-1 -mb-px">
            {(["payments", "schedules"] as const).map((tab) => (
              <button
                ...
                data-testid={
                  tab === "payments" ? "tab-my-payments" : "tab-my-schedules"
                }
        ...
        <div className="p-6">
          {isLoading ? (
            ...
            payments.length === 0 ? (
              ...
              <div className="overflow-x-auto" data-testid="my-payments-table">
                      ...
                      return (
                        ...
                        >
                          <td className="py-3 pr-6 text-gray-600 whitespace-nowrap">
                            {formatDate(
                              payment.date_paiement ?? payment.created_at,
                            )}
                          </td>
          ...
          schedules.length === 0 ? (
            ...
            <div className="overflow-x-auto" data-testid="my-schedules-table">
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="text-left text-xs font-medium text-gray-500 uppercase tracking-wide border-b border-gray-100">
