...
export function ChangeEmailSection({ currentEmail }: Props) {
  ...
  const formError =
    newEmail && !isValidNew
      ...
      ? t("profile.changeEmail.errorMismatch", "Les emails ne correspondent pas")
      : newEmail && !isDifferent
      ? t("profile.changeEmail.errorSameEmail", "Identique à l'adresse actuelle")
      : null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post("/auth/change-email", { newEmail: newEmail.trim().toLowerCase() });
      setSent(true);
      setNewEmail("");
      setConfirmEmail("");
      toast.success(
          ...
          <div className="flex items-start gap-3 rounded-lg bg-green-50 border border-green-200 px-4 py-3 text-sm text-green-800">
            ...
            <div>
              <p className="font-medium">
                {t("profile.changeEmail.sentTitle", "Email de confirmation envoyé")}
              </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            <input
              ...
              type="email"
              value={newEmail}
              onChange={(e) => { setNewEmail(e.target.value); setSent(false); }}
          ...
          <FormField
            id="confirm-email"
            label={t("profile.changeEmail.confirmEmail", "Confirmer le nouvel email")}
            required
            error={confirmEmail && !isMatch ? formError ?? undefined : undefined}
          >
            <input
              id="confirm-email"
              type="email"
