...
export const EditEventPage: React.FC = () => {
  ...
  const { getEvent, updateEvent, isUpdating, uploadEventImage, isUploadingImage } = useEvents();
  ...
  useEffect(() => {
    if (event) {
      setFormData({
        title: event.title || "",
        date: event.start_date ? new Date(event.start_date).toISOString().slice(0, 16) : "",
        capacity: event.capacity?.toString() || "",
        price: event.price?.toString() || "",
        visibility: event.visibility || "public",
        min_grade_id: event.min_grade_id?.toString() || "",
      });
    }
  }, [event]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) {
        await uploadEventImage({ eventId, file: imageFile });
      }
    ...
    <div className="space-y-6" data-testid="edit-event-page">
      <div className="flex items-center gap-4">
        ...
        <PageHeader
          ...
          description="Modifiez les informations de l'évènement."
