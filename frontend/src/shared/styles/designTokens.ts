...
export const TYPOGRAPHY = {
  ...
  labelRequired:
    'block text-sm font-medium text-gray-700 after:content-["*"] after:ml-0.5 after:text-red-500',
...
export const BUTTON = {
  ...
  variant: {
    ...
    danger:
      "text-white bg-red-600 hover:bg-red-700 focus:ring-red-500 shadow-sm",
    success:
...
export const MODAL = {
  ...
  // Header
  header:
    "flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100",
  ...
  animation: {
    ...
    content: {
      enter: "animate-in zoom-in-95 fade-in duration-200",
      exit: "animate-out zoom-out-95 fade-out duration-200",
    },
  },

  // Classes Tailwind pour animations custom (si besoin)
  overlayEnter: "transition-opacity duration-200 ease-out opacity-0",
  overlayEnterActive: "opacity-100",
  overlayExit: "transition-opacity duration-200 ease-in opacity-100",
  overlayExitActive: "opacity-0",

  contentEnter: "transition-all duration-200 ease-out opacity-0 scale-95",
  contentEnterActive: "opacity-100 scale-100",
  contentExit: "transition-all duration-200 ease-in opacity-100 scale-100",
  contentExitActive: "opacity-0 scale-95",
} as const;
...
export const FORM = {
  // Field wrapper
  field: "space-y-2",
  fieldInline: "flex items-center gap-4",
  ...
  labelRequired:
    'block text-sm font-medium text-gray-700 after:content-["*"] after:ml-0.5 after:text-red-500',
  labelOptional:
    'block text-sm font-medium text-gray-700 after:content-["(optionnel)"] after:ml-1 after:text-gray-400 after:font-normal after:text-xs',

  // Messages d'aide et d'erreur
  helpText: "text-xs text-gray-500 mt-1",
  errorText: "text-xs text-red-600 mt-1 flex items-center gap-1",
  successText: "text-xs text-green-600 mt-1 flex items-center gap-1",

  // SearchBar
  searchWrapper: "relative",
  searchIcon: "absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400",
  searchInput: "pl-10 pr-4",
  searchClearButton:
    "absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded-md hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors",

  // SelectField
  select:
    "block w-full px-3 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white transition-colors",
...
export const PAGINATION = {
  ...
  // Boutons prev/next
  navButton:
    "px-2.5 py-1.5 text-sm text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-md transition-colors disabled:opacity-40 disabled:cursor-not-allowed",
...
export function cardClass(
  variant: keyof typeof CARD.padding = "standard",
  withHover = false,
): string {
  return cn(CARD.base, CARD.padding[variant], withHover && CARD.hover);
}
...
export function badgeClass(
  ...
  return cn(BADGE.base, BADGE.variant[variant], BADGE.size[size]);
}
...
/**
 * Helper pour créer un FormField
 */
export function formFieldClass(inline = false): string {
  return inline ? FORM.fieldInline : FORM.field;
}

/**
 * Helper pour créer un label
 */
export function formLabelClass(required = false, optional = false): string {
...
/**
 * Helper pour créer un PageHeader
 */
export function pageHeaderClass(withStats = false): string {
  return cn(
    LAYOUT.pageHeader.container,
    withStats && "pb-4 border-b border-gray-200",
  );
}
