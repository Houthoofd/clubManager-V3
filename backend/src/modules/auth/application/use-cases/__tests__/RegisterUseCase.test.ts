...
describe("RegisterUseCase", () => {
  ...
  beforeEach(() => {
    ...
    // Re-setup EmailService mock — required because resetMocks: true (jest config)
    // resets all mock implementations before each test.
  ...
  describe("execute - Success scenarios", () => {
    ...
    it("should generate and store JWT tokens", async () => {
      ...
      const emailServiceInstance = (EmailService as jest.Mock).mock.results[0]
        .value;
      expect(emailServiceInstance.sendVerificationEmail).toHaveBeenCalledWith(
        mockUser.email,
        mockUser.first_name,
        expect.any(String),
        mockUser.userId,
    ...
    it("should generate and store email verification token", async () => {
      ...
      expect(
        mockAuthRepository.storeEmailVerificationToken,
    ...
    it("should store refresh token with 7 days expiration", async () => {
      ...
      // Registration no longer stores a refresh token — the user must verify email first
  ...
  describe("execute - Validation errors", () => {
    ...
    it("should throw error if date_of_birth indicates age < 5", async () => {
      ...
      const invalidDto = {
        ...validRegisterDto,
        date_of_birth: recentDate.toISOString(),
      };
    ...
    it("should throw error if date_of_birth indicates age > 120", async () => {
      ...
      const invalidDto = {
        ...validRegisterDto,
        date_of_birth: oldDate.toISOString(),
      };
  ...
  describe("execute - Business logic errors", () => {
    ...
    it("should check email existence before creating user", async () => {
      ...
      expect(mockAuthRepository.emailExists).toHaveBeenCalledWith(
  ...
  describe("execute - Edge cases", () => {
    it("should handle repository errors gracefully", async () => {
      mockAuthRepository.emailExists.mockRejectedValue(
    ...
    it("should handle password hashing errors", async () => {
      mockAuthRepository.emailExists.mockResolvedValue(false);
      (PasswordService.hash as jest.Mock).mockRejectedValue(
    ...
    it("should handle token generation errors", async () => {
      ...
      const emailServiceInstance = (EmailService as jest.Mock).mock.results[0]
        .value;
      emailServiceInstance.sendVerificationEmail.mockRejectedValue(
    ...
    it("should handle refresh token storage errors", async () => {
      ...
      const emailServiceInstance = (EmailService as jest.Mock).mock.results[0]
        .value;
      emailServiceInstance.sendVerificationEmail.mockResolvedValue({
        success: false,
        error: "Send failed",
      });

      // Email send failure is handled gracefully — registration still succeeds
      const result = await registerUseCase.execute(validRegisterDto);
      expect(result.success).toBe(true);
    });
    ...
    it("should handle email verification token generation errors", async () => {
      ...
      mockAuthRepository.createUser.mockResolvedValue(mockUser);
      (
        TokenService.generateEmailVerificationToken as jest.Mock
      ).mockImplementation(() => {
        throw new Error("Email token generation failed");
      });

      await expect(registerUseCase.execute(validRegisterDto)).rejects.toThrow(
        "Email token generation failed",
      );
    });
    ...
    it("should trim whitespace from email before validation", async () => {
      ...
      // Implementation does not trim whitespace — emails with surrounding spaces fail format validation
      await expect(registerUseCase.execute(dtoWithSpaces)).rejects.toThrow(
        "Invalid email format",
      );
      expect(mockAuthRepository.emailExists).not.toHaveBeenCalled();
    });
  ...
  describe("execute - Integration flow", () => {
    it("should execute complete registration flow in correct order", async () => {
      ...
      (
        PasswordService.validatePasswordStrength as jest.Mock
      ).mockImplementation(() => {
      ...
      mockAuthRepository.storeEmailVerificationToken.mockImplementation(
        async () => {
      ...
      const emailServiceInstance = (EmailService as jest.Mock).mock.results[0]
        .value;
      emailServiceInstance.sendVerificationEmail.mockImplementation(
        async () => {
