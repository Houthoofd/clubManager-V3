...
export const orderSchema = z.object({
    ...
    unique_id: z
        .string()
        .max(ORDER_CONSTRAINTS.UNIQUE_ID_MAX_LENGTH, `L'ID unique ne peut pas dépasser ${ORDER_CONSTRAINTS.UNIQUE_ID_MAX_LENGTH} caractères`)
        .nullable()
        .optional(),
    numero_commande: z
        .string()
        .max(ORDER_CONSTRAINTS.NUMERO_COMMANDE_MAX_LENGTH, `Le numéro de commande ne peut pas dépasser ${ORDER_CONSTRAINTS.NUMERO_COMMANDE_MAX_LENGTH} caractères`)
        .nullable()
        .optional(),
    ...
    ip_address: z
        .string()
        .max(ORDER_CONSTRAINTS.IP_ADDRESS_MAX_LENGTH, `L'adresse IP ne peut pas dépasser ${ORDER_CONSTRAINTS.IP_ADDRESS_MAX_LENGTH} caractères`)
        .ip({ version: 'v4' })
    ...
    user_agent: z
        .string()
        .max(ORDER_CONSTRAINTS.USER_AGENT_MAX_LENGTH, `Le user agent ne peut pas dépasser ${ORDER_CONSTRAINTS.USER_AGENT_MAX_LENGTH} caractères`)
        .nullable()
        .optional(),
