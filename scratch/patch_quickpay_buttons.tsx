...
// Composant pour l'animation de succès (SVG animé)
...
const InlineStripeForm: React.FC<{
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 p-6 bg-white rounded-xl border border-gray-200 shadow-sm relative">
      ...
      {/* On utilise layout accordion pour mieux séparer Link de la carte classique */}
      ...
      {/* Bouton unique, de la même largeur que les éléments Stripe */}
      <div className="mt-6">
        <button 
          type="submit" 
        ...
        >
          {isLoading ? "Traitement en cours..." : `Payer ${formatCurrency(amount)}`}
        </button>
      </div>
    </form>
  );
