...
export class PaymentController {
  ...
  async createPublicStripeIntent(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (item_type === "boutique") {
        ...
        const order = await orderRepo.findById(commande_id);
        if (!order) throw new Error("Commande introuvable");
        montant = order.total;
        description = "Commande N°" + order.numero_commande;
      } else {
        echeance_id = Number(item_id);
        const schedule = await scheduleRepo.findById(echeance_id);
        if (!schedule) throw new Error("Échéance introuvable");
        montant = schedule.montant;
        description = schedule.description || "Cotisation";
      }
  ...
  /**
   * Endpoint PUBLIQUE pour vérifier le statut d'un paiement manuellement (fallback si le webhook échoue ou est retardé).
   */
